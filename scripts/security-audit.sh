...
if rg --hidden --glob '!node_modules/**' --glob '!**/dist/**' --glob '!**/.expo/**' --glob '!**/.turbo/**' \
  ...
  '(-----BEGIN (RSA |OPENSSH |EC |DSA |PRIVATE )?PRIVATE KEY-----|SUPABASE_SERVICE_ROLE_KEY=.+|EXPO_PUBLIC_SUPABASE_ANON_KEY=.+|sk_live_|sk_test_|ghp_[A-Za-z0-9_]+|github_pat_[A-Za-z0-9_]+|AIza[0-9A-Za-z_-]{35})' .; then
